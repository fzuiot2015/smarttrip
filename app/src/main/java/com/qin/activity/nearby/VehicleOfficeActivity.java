...
import com.qin.adapter.recyclerview.usersearch.VehicleRecyclerViewAdapter;
...
import com.qin.pojo.usersearch.vihecle.Vihecle;
...
public class VehicleOfficeActivity extends AppCompatActivity implements
    ...
    @BindView(R.id.ll_vehicle_about)
    LinearLayout mLlEhicleAbout;
    @BindView(R.id.rlv_vehicle)
    RecyclerView mRlvEhicle;
    @BindView(R.id.refresh_vehicle)
    SmartRefreshLayout mRefreshEhicle;
    @BindView(R.id.tv_vehicle_no)
    TextView mTvEhicleNo;
    ...
    private VehicleRecyclerViewAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_vehicleoffice);
        ...
        mRefreshEhicle.setOnRefreshLoadMoreListener(this);
        mRefreshEhicle.setPrimaryColorsId(R.color.colorGreen, android.R.color.white);

        initPopWindow(this, R.layout.popwindow_search_bound, mWidthPixels, LinearLayout.LayoutParams.WRAP_CONTENT);

        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        mRlvEhicle.setItemAnimator(new DefaultItemAnimator());
        mRlvEhicle.addItemDecoration(new MarginDecorationVertical(20));
        mRlvEhicle.setLayoutManager(layoutManager);
    ...
    @OnClick(R.id.ll_vehicle_about)
    public void onViewClicked(View view) {
        isLoadMore = false;
        switch (view.getId()) {
            case R.id.ll_vehicle_about:
                if (popIsShowing == 2) {
                    popupWindow.dismiss();
                    popIsShowing = 1;
                    ivSearchNearbypoiAbout.setImageResource(R.mipmap.up_login);
                } else {
                    popupWindow.showAsDropDown(mLlEhicleAbout, 0, 30);
                    setWindowGray(0.7f);
    ...
    public static void startActivityWithParams(Context context, Intent intent, int number, double lat, double lon) {
        ...
        intent.setClass(context, VehicleOfficeActivity.class);
    ...
    private void accessNetParking(int radius, int pageNum) {
        ...
        upParking.setTags("交通警察");
        upParking.setPage_num(pageNum + "");
        Gson gson = new Gson();
        String json = gson.toJson(upParking);
        OkGo.<String>post(ConstantValues.URL_VEHICLE).tag(this).upJson(json).execute(new StringCallback() {
            @Override
    ...
    private void accessNetParkingLoadMore(int radius, int pageNum) {
        ...
        OkGo.<String>post(ConstantValues.URL_VEHICLE).tag(this).upJson(json).execute(new StringCallback() {
            ...
            public void onSuccess(Response<String> response) {
                ...
                loadingLayout.setStatus(LoadingLayout.Success);
                Gson gson = new Gson();
                Vihecle vihecle = gson.fromJson(body, Vihecle.class);
                List<Results> results = vihecle.getResults();
                if (results.size() == 0) {
                    ToastUtils.showBgResource(VehicleOfficeActivity.this, "已加载所有数据");
                } else {
                    mResultsList.addAll(results);
                    mAdapter.notifyDataSetChanged();
                }
                mRefreshEhicle.finishLoadMore();
            }
...
//                refreshParking.setVisibility(View.GONE);
    ...
    private void parseDataParking(String json) {
        try {
            ...
            if (status.equals("0")) {
                Gson gson = new Gson();
                Vihecle vihecle = gson.fromJson(json, Vihecle.class);
                mResultsList = vihecle.getResults();
                //TODO  明天判断返回集合是否为空
                mAdapter = new VehicleRecyclerViewAdapter(mResultsList);
                mRlvEhicle.setAdapter(mAdapter);
                mAdapter.notifyDataSetChanged();
                mAdapter.setOnItemClickListener(new VehicleRecyclerViewAdapter.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, VehicleRecyclerViewAdapter.ViewName viewName, int postion) {
                        if (VehicleRecyclerViewAdapter.ViewName.ITEM == viewName) {
                            AmapNaviPage.getInstance().showRouteActivity(getApplicationContext()
                                    ...
                                    , VehicleOfficeActivity.this);
                        }
                        if (VehicleRecyclerViewAdapter.ViewName.IMAGEVIEW == viewName) {
                            ToastUtils.showBgResource(VehicleOfficeActivity.this, "跳转" + postion);
                        }
                    }
                });
            } else {
                ToastUtils.showBgResource(VehicleOfficeActivity.this, message);
            }
        } catch (Exception e) {
            e.printStackTrace();
            ToastUtils.showBgResource(VehicleOfficeActivity.this, "获取异常");
        }
    }
